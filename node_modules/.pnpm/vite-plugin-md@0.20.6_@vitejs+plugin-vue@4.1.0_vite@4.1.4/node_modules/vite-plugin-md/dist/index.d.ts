...
declare type PipelineInitializer = (i?: Pipeline<PipelineStage.initialize>) => Pipeline<PipelineStage.initialize>;
declare type Parser<S extends IPipelineStage> = S extends 'parser' | 'parsed' | 'dom' | 'sfcBlocksExtracted' | 'closeout' ? {
...
declare type Initialization<S extends IPipelineStage> = S extends 'initialize' ? {
...
declare type MetaExtracted<S extends IPipelineStage> = S extends 'initialize' ? {} : {
...
declare type HtmlContent<S extends IPipelineStage> = S extends 'parsed' | 'sfcBlocksExtracted' | 'closeout' ? {
    /**
...
declare type Blocks<S extends IPipelineStage> = S extends 'sfcBlocksExtracted' | 'closeout' ? {
...
declare type Completed<S extends IPipelineStage> = S extends 'closeout' ? {
    /** the finalized component in string form */
    component: string;
    /** The sourcemap from Markdown to SFC */
    map?: ExistingRawSourceMap | undefined;
} : {};
declare type Pipeline<S extends IPipelineStage> = {
 ...
 * - a `Pipeline<S>` value
 */
declare type PipeEither<S extends IPipelineStage> = Either<string, Pipeline<S>>;
/**
 * Carries an `TaskEither<T>` condition which is either:
 * - a _string_ error condition
 * - a `() => Promise<Pipeline<S>>` task
 */
declare type PipeTask<S extends IPipelineStage> = TE.TaskEither<string, Pipeline<S>>;
/**
 * A pipeline payload or either an async `PipeTask<S>` or a synchronous `PipeEither<S>`
 */
declare type PipelinePayload<S extends IPipelineStage> = PipeTask<S> | PipeEither<S>;
/**
 ...
 */
declare type SyncPipelineTransformer<F extends IPipelineStage, T extends IPipelineStage> = (payload: PipeTask<F>) => PipeTask<T>;
/**
...
*/
declare type AsyncPipelineTransformer<F extends IPipelineStage, T extends IPipelineStage> = (payload: PipeTask<F>) => PipeTask<T>;

declare type ViteConfig = Parameters<Exclude<Plugin['configResolved'], undefined>>[0];
/**
 ...
 * ```
 */
declare type ExcerptFunction = ((contents: string, meta: ExcerptMeta) => string) | ((contents: string) => string);
/**
 ...
 * ```
 */
declare type FmValueCallback = (fm: Frontmatter, filename: string) => Frontmatter;
/**
 * Values allowed to be set as frontmatter props
 */
declare type FmAllowedValue = string | number | undefined | any[] | Symbol | boolean;
/**
...
declare type WithConfig<T extends ResolvedOptions> = ViteConfigPassthrough & T;
declare type ReturnValues = string | string[] | number | boolean | Object;
